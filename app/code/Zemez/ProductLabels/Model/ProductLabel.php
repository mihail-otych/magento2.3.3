...
use Zemez\ProductLabels\Api\Data\ProductLabelInterface;
...
class ProductLabel extends \Magento\Rule\Model\AbstractModel implements ProductLabelInterface
{
    ...
    public function __construct(
        ...
        \Zemez\ProductLabels\Model\Filter\UseDateRange $filterDataRange,
        \Zemez\ProductLabels\Model\Filter\IsOnSale $isOnSale,
        \Zemez\ProductLabels\Model\Filter\IsNew $isNew,
        \Zemez\ProductLabels\Model\Filter\Price $price,
        \Zemez\ProductLabels\Model\Filter\Stock $stock,
        \Zemez\ProductLabels\Model\Indexer\Label\SmartLabel\SmartLabelProductProcessor $ruleProductProcessor,
    ...
    {
        $this->_init('Zemez\ProductLabels\Model\ResourceModel\ProductLabel');
    }
    ...
    protected function _getFilteredCollection($customerGroupId = null, $websiteId = null)
    {
        ...
        if ($this->getStockStatus() == self::STATUS_YES) {
        ...
        if ($this->getUsePriceRange()) {
            $this->_price->addPriceFilter(
                $productCollection, $this->getByPrice(), $this->getFromPrice(),
                $this->getToPrice()
            );
        }
    ...
    public function callbackValidateProduct($args)
    {
        ...
        foreach ($websites as $websiteId => $defaultStoreId) {
            ...
            if ($conform = $this->getConditions()->validate($product)) {
                $results[$websiteId] = $conform;
    ...
    public function afterSave()
    {
        ...
        if (!empty($this->_productIds) && is_array($this->_productIds)) {
            ...
            $this->_ruleProductProcessor->reindexList([]);
        }
