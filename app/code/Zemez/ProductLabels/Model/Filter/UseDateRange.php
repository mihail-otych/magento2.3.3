...
class UseDateRange
{
    ...
    /**
     * @param \Zemez\ProductLabels\Model\ProductLabel $rule
     *
     * @return array
     */
    public function getDateRangeCondition(\Zemez\ProductLabels\Model\ProductLabel $rule)
    {
        $dateCondition = [];
        if ($rule->getUseDateRange()) {
            ...
            $fromDateFormat =
                $this->_localeDate->date($fromDate)->setTime($fromTimeArr[0], $fromTimeArr[1], 0)->format(
                    'Y-m-d H:i:s'
                );
            $toDateFormat =
