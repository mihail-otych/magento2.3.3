...
class Rewards extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function getStatistic($customerId)
    {
        ...
        $result[self::BALANCE] = $result[self::REWARDED] - $result[self::REDEEMED] - $result[self::EXPIRED];
    ...
    public function getTotalDataByParams($select)
    {
        ...
        $select
            ...
            ->columns([
                ...
                self::REDEEMED => 'ABS(SUM(IF(main_table.amount < 0 AND main_table.action != \'Expiration\', main_table.amount, 0)))', // TODO: Use action code in database
                self::EXPIRED => 'ABS(SUM(IF(main_table.amount < 0 AND main_table.action = \'Expiration\', main_table.amount, 0)))', // TODO: Use action code in database
            ]);
